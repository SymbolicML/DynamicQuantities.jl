...
"""
    uexpand(q::AbstractUnionQuantity{<:Any,<:SymbolicDimensions})
...
"""
function uexpand(q::Q) where {T,R,D<:SymbolicDimensions{R},Q<:AbstractUnionQuantity{T,D}}
...
"""
    uconvert(qout::AbstractUnionQuantity{<:Any, <:SymbolicDimensions}, q::AbstractUnionQuantity{<:Any, <:Dimensions})
...
"""
function uconvert(qout::AbstractUnionQuantity{<:Any, <:SymbolicDimensions}, q::AbstractUnionQuantity{<:Any, <:Dimensions})
...
function uconvert(qout::AbstractUnionQuantity{<:Any,<:SymbolicDimensions}, q::QuantityArray{<:Any,<:Any,<:Dimensions})
    @assert isone(ustrip(qout)) "You passed a quantity with a non-unit value to uconvert."
    qout_expanded = uexpand(qout)
    dimension(q) == dimension(qout_expanded) || throw(DimensionError(q, qout_expanded))
...
"""
    uconvert(qout::AbstractUnionQuantity{<:Any, <:SymbolicDimensions})
...
"""
uconvert(qout::AbstractUnionQuantity{<:Any, <:SymbolicDimensions}) = Base.Fix1(uconvert, qout)
