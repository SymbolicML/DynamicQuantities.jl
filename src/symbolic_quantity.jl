...
static_fieldnames(::Type{<:SymbolicDimensions}) = UNIT_SYMBOLS
Base.getproperty(d::SymbolicDimensions{R}, s::Symbol) where {R} = data(d)[UNIT_MAPPING[s]]
    ...
    function _generate_unit_symbols()
        UNIT_SYMBOLS_EXIST[] || lock(UNIT_SYMBOLS_LOCK) do
            ...
            for unit in UNIT_SYMBOLS
                @eval const $unit = Quantity(1.0, SymbolicDimensions{DEFAULT_DIM_BASE_TYPE}; $(unit)=1)
            end
