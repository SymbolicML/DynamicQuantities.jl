...
Base.convert(::Type{AF}, x::FixedRational{T,den}) where {AF<:AbstractFloat,T,den} = convert(AF, convert(Rational, x))
Base.round(::Type{T}, x::F) where {T,T2,den,F<:FixedRational{T2,den}} = round(T, convert(Rational, x))
